...
const NavCircle = ({children}) => {
    ...
    return (
        <div id="hdr" className={menuOpen && "open"}>
			<div onClick={()=>setMenuOpen(!menuOpen)} className={`toggle-button  ${magnetClass && "magnet"}`} style={{	'transform' : 'translate('+trns_x+'px,'+trns_y+'px)'}}>
				<span></span>
				<span></span>
				<span></span>
				<span></span>
					...
					{len ? (
						<Fragment>
							{children.map((child,idx)=>(
								<li onClick={()=>setMenuOpen(false)} key={idx} onMouseOver={()=> hoverFunc(idx)} className={activeIndex === idx ? "list_item active" : "list_item "}>{child}</li>
							))}
						</Fragment>
					) : null}
				</ul>
